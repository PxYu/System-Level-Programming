								   ...
								   const char *szModule ) 
{
	...
	sprintf_s( rcIndProgID, MAX_LENGTH, "%s.%s", szProgIDPrefix, szClassProgID ) ;
	sprintf_s( rcProgID, MAX_LENGTH, "%s.%d", rcIndProgID, iVersion );
									 ...
									 const char *szClassProgID )
{
	...
	char rcProgID[ONE28];	   // szProgIDPrefix.szClassProgID
	char rcIndProgID[ONE28]; // rcProgID.iVersion


	// format the prog ID values.
	sprintf_s( rcProgID, ONE28, "%s.%s", szProgIDPrefix, szClassProgID );
	sprintf_s( rcIndProgID, ONE28, "%s.%d", rcProgID, iVersion );
